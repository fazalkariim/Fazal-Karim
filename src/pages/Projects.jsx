...
import { ProjectCard, projectsData } from "../components/sections/ProjectCard.jsx";
...
export default function Projects() {
  ...
  return (
    <>
      <PageHeader
        eyebrow="Portfolio"
        title="Projects that"
        highlight="shipped & scaled."
        description="A selection of recent client work, side projects, and product experiments."
      />
      ...
      <section className="py-12">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <div className="relative rounded-3xl border border-border bg-card overflow-hidden grid lg:grid-cols-2 gap-0">
           ...
           <div className={`relative aspect-video lg:aspect-auto bg-gradient-to-br ${featured.accent}`}>
  ...
  {featured.image ? (
    <img
      src={featured.image}
      alt={featured.title}
      className="relative z-10 h-full w-full object-contain ml-5 "
    />
  ) : (
      ...
      <span className="text-6xl font-extralight text-foreground/40 tracking-tight">
        {featured.title}
            ...
            <div className="p-10 lg:p-14 flex flex-col justify-center">
              ...
              <h2 className="text-3xl md:text-4xl font-light tracking-tight mb-4">{featured.title}</h2>
              <p className="text-muted-foreground font-light leading-relaxed mb-6">{featured.description}</p>
              <div className="flex flex-wrap gap-2">
                {featured.tech.map((t) => (
                  <span key={t} className="text-[10px] uppercase tracking-widest px-2.5 py-1 rounded-full bg-surface border border-border text-muted-foreground">
                    {t}
                  </span>
                ))}
              </div>
